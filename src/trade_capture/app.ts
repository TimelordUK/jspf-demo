...
function describeResilientRun (opts: CliOptions, hasServer: boolean): void {
  ...
  if (!hasServer) {
    ...
    console.log(`    'retries N'                          the attempts, one every ${retryEvery}s`)
    console.log('')
    console.log('  those retry lines carry no reason on node 20+: a host that resolves to both ::1')
    console.log('  and 127.0.0.1 fails as an AggregateError, whose own message is empty and whose')
    console.log('  detail sits in errors[].  jspurefix logs the message, so it prints nothing.')
    console.log('')
